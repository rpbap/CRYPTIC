#!/usr/bin/env python3
# CRYPTIC v1.2.0: Cryptosporidium gp60 Typing and Identification Classifier
...
class BlastHit:
    ...
    qseq: str
    sseq: str
    qcov_override: Optional[float] = None
    scov_override: Optional[float] = None
    hsp_count: int = 1

    @property
    def qcov(self) -> float:
        # BLAST alignment length includes gaps, so length/qlen can exceed 100%.
        # Use the actual query-coordinate span unless an HSP-union value was set.
        if self.qcov_override is not None:
            return max(0.0, min(100.0, self.qcov_override))
        if not self.qlen:
            return 0.0
        aligned_bp = abs(self.qend - self.qstart) + 1
        return max(0.0, min(100.0, 100.0 * aligned_bp / self.qlen))

    @property
    def scov(self) -> float:
        # Same correction for subject/reference coverage.
        if self.scov_override is not None:
            return max(0.0, min(100.0, self.scov_override))
        if not self.slen:
            return 0.0
        aligned_bp = abs(self.send - self.sstart) + 1
        return max(0.0, min(100.0, 100.0 * aligned_bp / self.slen))
...
def collapse_genome_hsps(hits: List[BlastHit], max_subject_gap: int = 500) -> List[BlastHit]:
    """
    Collapse compatible BLAST HSPs for the same gp60 reference/locus.

    gp60 contains informative tandem repeats. BLAST can split a real full-length
    locus into adjacent HSPs when repeat counts differ from a database reference.
    Genome mode therefore combines HSPs only when they are on the same reference,
    same contig, same strand, and within a local genomic cluster. Distant repeat-only
    matches are kept separate and cannot inflate locus coverage.
    """
    grouped: Dict[Tuple[str, str, str], List[BlastHit]] = defaultdict(list)
    for hit in hits:
        grouped[(hit.qseqid, hit.sseqid, hit.strand)].append(hit)

    collapsed: List[BlastHit] = []
    for (_qseqid, _sseqid, strand), group in grouped.items():
        group = sorted(group, key=lambda h: min(h.sstart, h.send))
        clusters: List[List[BlastHit]] = []
        ...
        for hit in group:
            ...
            allowed_gap = max(max_subject_gap, int(hit.qlen * 0.5))
            if current and current_end is not None and s_lo > current_end + allowed_gap:
                clusters.append(current)
                current = []
                current_end = None
            current.append(hit)
            current_end = s_hi if current_end is None else max(current_end, s_hi)
        if current:
            clusters.append(current)

        for cluster in clusters:
            # Prefer stronger HSPs, and only credit query/reference bases that add
            # new coverage. This prevents overlapping repeat-only HSPs from being
            # counted multiple times.
            covered: List[Tuple[int, int]] = []
            selected: List[Tuple[BlastHit, int]] = []
            for hit in sorted(cluster, key=lambda h: (h.bitscore, h.length, h.pident), reverse=True):
                interval = (min(hit.qstart, hit.qend), max(hit.qstart, hit.qend))
                before = interval_union_length(covered)
                after = interval_union_length(covered + [interval])
                added_bp = after - before
                if added_bp <= 0:
                    continue
                selected.append((hit, added_bp))
            ...
            first = selected[0][0]
            covered_bp = interval_union_length(covered)
            weight = sum(added for _hit, added in selected)
            pident = sum(hit.pident * added for hit, added in selected) / weight
            qcov = 100.0 * covered_bp / first.qlen if first.qlen else 0.0

            subject_lo = min(min(hit.sstart, hit.send) for hit, _added in selected)
            subject_hi = max(max(hit.sstart, hit.send) for hit, _added in selected)
            if strand == "minus":
                sstart, send = subject_hi, subject_lo
            else:
                sstart, send = subject_lo, subject_hi

            q_lo = min(min(hit.qstart, hit.qend) for hit, _added in selected)
            q_hi = max(max(hit.qstart, hit.qend) for hit, _added in selected)
            mismatch = int(round(sum((100.0 - hit.pident) * added / 100.0 for hit, added in selected)))

            collapsed.append(BlastHit(
                qseqid=first.qseqid,
                sseqid=first.sseqid,
                pident=pident,
                length=covered_bp,
                mismatch=mismatch,
                gapopen=sum(hit.gapopen for hit, _added in selected),
                qstart=q_lo,
                qend=q_hi,
                sstart=sstart,
                send=send,
                evalue=min(hit.evalue for hit, _added in selected),
                # Do not reward fragmented alignments by summing bit scores.
                bitscore=max(hit.bitscore for hit, _added in selected),
                qlen=first.qlen,
                slen=first.slen,
                qseq="",
                sseq="",
                qcov_override=min(100.0, qcov),
                hsp_count=len(selected),
            ))
...
def pick_best_hits(
    ...
    coverage_attr: str = "qcov",
    max_identity_delta: float = 0.25,
    max_coverage_delta: float = 2.0,
) -> Tuple[Optional[BlastHit], List[BlastHit]]:
    """Pick the best hit and only genuinely near-indistinguishable competitors.

    A competing reference may downgrade confidence only if it passes the same
    identity/coverage thresholds as the best call and is very close in both
    identity and coverage. This is important for gp60 because repeat differences
    are subtype-informative and should not be erased by a composite score tie.
    """
    if not hits:
        return None, []
    ...
    best_cov = float(getattr(best, coverage_attr))
    best_is_confident = (
        (min_pident is None or best.pident >= min_pident)
        and (min_coverage is None or best_cov >= min_coverage)
    )

    close: List[BlastHit] = []
    for hit in ranked:
        if best_score - hit_rank_score(hit) > max_delta:
            continue
        if best_is_confident:
            hit_cov = float(getattr(hit, coverage_attr))
            if min_pident is not None and hit.pident < min_pident:
                continue
            if min_coverage is not None and hit_cov < min_coverage:
                continue
            if abs(best.pident - hit.pident) > max_identity_delta:
                continue
            if abs(best_cov - hit_cov) > max_coverage_delta:
                continue
        close.append(hit)

    if best not in close:
        close.insert(0, best)
    return best, close
...
def type_assemblies(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for idx, genome in enumerate(genomes, start=1):
            ...
            log(f"[{idx}/{len(genomes)}] BLAST HSPs found: {len(hits)}", verbose)
            locus_hits = collapse_genome_hsps(hits)
            log(f"[{idx}/{len(genomes)}] Local reference/locus candidates after HSP collapsing: {len(locus_hits)}", verbose)
            best, close = pick_best_hits(
                locus_hits,
                max_delta=ambiguity_delta,
                min_pident=min_pident,
                min_coverage=min_qcov,
                coverage_attr="qcov",
            )
            ...
            if best.hsp_count > 1:
                extra = f"combined {best.hsp_count} compatible HSPs across the gp60 locus"
...
def type_extracted_gp60(
    ...
    for idx, qid in enumerate(query_ids, start=1):
        ...
        best, close = pick_best_hits(qhits, max_delta=ambiguity_delta, min_pident=min_pident, min_coverage=min_qcov, coverage_attr="scov")
...
def classify_consensus_sequence(sample_id: str, consensus_seq: str, db_path: Path, threads: int, min_qcov: float, min_pident: float, ambiguity_delta: float) -> Dict[str, str]:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        hits = parse_blast_tsv(blast_tsv)
        best, close = pick_best_hits(hits, max_delta=ambiguity_delta, min_pident=min_pident, min_coverage=min_qcov, coverage_attr="scov")
        if best is None:
